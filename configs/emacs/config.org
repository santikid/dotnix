...
(defun my/remote-directory-p (&optional directory)
  "Return non-nil when DIRECTORY is remote."
...
(leader-def
  ...
  "bo" '(my/kill-other-file-buffers :which-key "delete other file buffers")
  ...
  "cr" '(my/rename-current-file :which-key "rename file")
...
(use-package consult
  ...
  (defun my/consult-todos ()
    "Search project TODO-like comments."
    (interactive)
    (consult-ripgrep nil "\\b\\(TODO\\|FIXME\\|HACK\\|NOTE\\):"))
...
(use-package forge
  ...
  :general
  (leader-def
    "gf" '(:ignore t :which-key "forge")
    "gff" '(forge-dispatch :which-key "forge menu")
    "gfp" '(forge-list-pullreqs :which-key "list PRs")
    "gfi" '(forge-list-issues :which-key "list issues")
    "gfc" '(forge-create-pullreq :which-key "create PR")
    "gfC" '(forge-create-issue :which-key "create issue")
    "gfF" '(forge-pull :which-key "fetch forge data")
    "gfb" '(forge-browse-remote :which-key "browse remote")
    "gfB" '(forge-browse-topic :which-key "browse PR/issue")))
#+end_src
...
(use-package vterm
  ...
  (defun my/project-name-for-directory (directory)
    "Return a compact project name for DIRECTORY."
    (let* ((remote-host (file-remote-p directory 'host))
           (local-name (directory-file-name (file-local-name directory)))
           (leaf (file-name-nondirectory local-name)))
      (cond
       ((and remote-host (not (string= leaf "")))
        (format "%s:%s" remote-host leaf))
       (remote-host remote-host)
       ((not (string= leaf "")) leaf)
       (t "default"))))

  (defun my/shell-buffer-name (directory)
    "Return a shell buffer name for DIRECTORY."
    (format "*shell-%s*" (my/project-name-for-directory directory)))

  (defun my/vterm-project ()
    "Open or switch to project-specific vterm terminal."
    (interactive)
    (let* ((default-directory (my/project-root-or-default))
           (project-name (my/project-name-for-directory default-directory))
           (buffer-name (format "*vterm-%s*" project-name)))
      (if (my/remote-directory-p default-directory)
          (my/project-shell)
  ...
  (defun my/project-shell-buffer ()
    "Create or return a project shell buffer."
    (let* ((default-directory (my/project-root-or-default))
           (buffer-name (my/shell-buffer-name default-directory)))
      (shell buffer-name)
      (get-buffer buffer-name)))

  (defun my/project-shell ()
    "Open a project shell, using TRAMP when `default-directory' is remote."
    (interactive)
    (switch-to-buffer (my/project-shell-buffer)))

  (defun my/terminal ()
    "Open a terminal appropriate for the current directory."
    (interactive)
    (let ((default-directory (my/project-root-or-default)))
      (if (my/remote-directory-p default-directory)
          (my/project-shell)
  ...
  (defun my/terminal-other-window ()
    "Open a terminal in another window."
    (interactive)
    (let ((default-directory (my/project-root-or-default)))
      (if (my/remote-directory-p default-directory)
          (switch-to-buffer-other-window (my/project-shell-buffer))
        (vterm-other-window))))

  (defun my/vterm-list ()
    "List and switch to vterm terminals."
    (interactive)
    (let ((vterm-buffers (seq-filter
      ...
      (if vterm-buffers
          (switch-to-buffer
           (completing-read "Terminal: " vterm-buffers))
        (message "No vterm terminals open"))))

  :general
  (leader-def
    "t" '(:ignore t :which-key "terminal")
    "tt" 'my/terminal
    "tT" 'my/terminal-other-window
    "tp" 'my/vterm-project
    "ts" 'my/project-shell
    "tv" 'vterm
    "tl" 'my/vterm-list))
...
(use-package treemacs
  ...
  ;; Allow C-h/j/k/l window navigation in treemacs
...
(use-package org
  ...
  (leader-def
    ...
    "oa" 'org-agenda
    "oc" 'org-capture
    "of" '(my/find-org-file :which-key "find org file")
    "ol" 'org-store-link
    "or" 'org-refile))
#+end_src
...
(use-package org-roam
  ...
  (leader-def
    ...
    "nt" 'org-roam-dailies-goto-tomorrow))
#+end_src
